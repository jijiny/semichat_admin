...
<table class="table" style=" width: 80%; height:500px; margin: 0 auto; margin-top: 10px;  ">
	<tr style="background-color: lightgray;">
		<th style="text-align: left">${board.iBoardNo }</th>
		<th>${board.iBoardTitle }</th>
		<th>${board.counselorId }</th>
		<th>${board.iBoardDate }</th>
		<th>${board.iBoardViews }</th>
	</tr>
	<tr>
...
<%-- 		<textarea cols="110px;" rows="15px;" disabled="disabled" style="resize: none; background-color: white;"><%=board.getiBoardContent() %></textarea> --%>
		<div style=" height:500px; background-color: white; padding:10px;">${board.iBoardContent }</div>
		</td>
	</tr>	
	<tr>
		<td>첨부파일 </td>
...
<hr>
<br>
<button class="btn btn-md b-btn" style="float: left; background-color: #4ea1d3; color: white; "onclick="location.href='/admin/inquirymanage';">목록</button>
<%-- <a href="/inquiry/modify?iboardno=<%=board.getiBoardNo()%>"> --%>

<button class="btn btn-md b-btn" style="float: right; margin-right: 5px; background-color: #4ea1d3; color: white;" id="btnUpdate" onclick="location.href='/admin/reply?iboardNo=${board.iBoardNo}'">답변</button>
